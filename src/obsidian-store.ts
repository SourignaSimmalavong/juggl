...
${edge.data.context}`;
  ...
  async refreshNode(id: VizId, view: IJuggl) {
    ...
    const idS = id.toId();
    let correctEdges: IMergedToGraph | undefined;
    ...
    if (node.length > 0 && node.hasClass(CLASS_EXPANDED)) {
      ...
      // const all_nodes = view.viz.nodes();
      // let self = this;
      // let all_nodes_def: NodeDefinition[] = await Promise.all(all_nodes.map(async (n) =>new Promise(async res => {
      //   return await self.get(VizId.fromNode(n), view);
      // })));
      // all_nodes_def.push(nodeDef);
      // view.mergeToGraph(all_nodes_def, true, false);
    ...
    let removed;
    if (correctEdges) {
      removed = node.connectedEdges()
        .difference(correctEdges.merged)
        .remove();
    }
    if ((removed && removed.length > 0) || (correctEdges && correctEdges.added.length > 0)) {
      view.onGraphChanged(true, true);
    }
  }
